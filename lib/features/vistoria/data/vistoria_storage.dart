...
class VistoriaStorage {
  ...
  Future<void> updateStatus(String id, SyncStatus status) async {
    ...
    final vistoria =
        Vistoria.fromJson(jsonDecode(json) as Map<String, dynamic>);
    await box.put(id, jsonEncode(vistoria.copyWith(syncStatus: status).toJson()));
  }

  Future<void> updateResultados(
      String id, List<ItemResult> resultados) async {
    final box = await _box;
    final json = box.get(id);
    if (json == null) return;
    final vistoria =
        Vistoria.fromJson(jsonDecode(json) as Map<String, dynamic>);
    await box.put(
        id, jsonEncode(vistoria.copyWith(resultados: resultados).toJson()));
  }
}
